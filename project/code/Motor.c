...
int8 dir = 1;
int16 duty = 0;
// 在时钟初始化前负压电机GPIO拉低，PWM初始占空比调整为10000//
// Motor para Control Area//
void Motor_Init(void)
{
    
    gpio_init(nSleep1, GPO, GPIO_LOW, GPO_PUSH_PULL);  // 先拉低
    gpio_init(nSleep2, GPO, GPIO_LOW, GPO_PUSH_PULL);
...
system_delay_ms(20);
	    ...
	    gpio_init(nSleep2, GPO, GPIO_HIGH, GPO_PUSH_PULL); // GPIO 初始化为输出 默认上拉输出高
...
void Motor_fuya(void)
{

    gpio_set_level(DIR_3, GPIO_HIGH);                 // DIR输出高电平
    pwm_set_duty(PWM_3, fy_duty * (PWM_DUTY_MAX / 100)); // 计算占空比

    if (dir) // 根据方向判断计数方向 本例程仅作参考
    {
        fy_duty++;               // 正向计数
        if (fy_duty >= MAX_DUTY) // 达到最大值
            fy_duty = MAX_DUTY;  // 变更计数方向
